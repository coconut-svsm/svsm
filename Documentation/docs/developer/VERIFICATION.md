...
* Specifications for dependencies (external crates): If functions, structs, or
  ...
  specifications in `lib/verify_external/`.
* Performance: Store expensive and reusable proofs in `lib/verify_proof/` if
  not provided by `vstd`. The `svsm/kernel/build.rs` sets `rlimit=1`, while
  `lib/verify_proof/build.rs` sets `rlimit=4`, helping developers decide when
  they need more proof engineering to run verification within minutes. For some
  proofs that are tightly related to the project code, we may still put them
  under svsm/kernel with `#[verus_verify(rlimit=x)]`
