...
const SVSM_UEFI_MM_REGISTER_BUFFER: u32 = 1;
...
fn uefi_mm_request_doorbell(_params: &RequestParams) -> Result<(), SvsmReqError> {
    let buffer = BUFFER.lock();
    let mut store = STORE.lock();

    // check buffer parameters
    check_buffer(buffer.addr, buffer.size)?;
...
pub fn uefi_mm_protocol_request(
    ...
    match request {
        ...
        SVSM_UEFI_MM_REQUEST_DOORBELL => uefi_mm_request_doorbell(params),
