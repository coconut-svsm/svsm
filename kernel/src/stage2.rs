...
fn prepare_kernel_image(
    ...
    if private_pte_mask() != 0 {
        for pt_index in 0..launch_info.kernel_pt_pages {
            ...
            map_page_range(MemoryRegion::new(vaddr, PAGE_SIZE), PhysAddr::from(paddr))?;
            ...
            // construction.
            let mut page_table_page = unsafe { KernelPageTablePage::new(vaddr) };
...
pub extern "C" fn stage2_main(launch_info: &Stage2LaunchInfo, vtom: usize) -> ! {
    ...
    // Obtain a mapping of the kernel launch info so it can be modified based
