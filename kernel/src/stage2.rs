...
unsafe fn setup_env(
    ...
    // SAFETY: the CPUID page address was correctly configured by the caller.
...
/// # Safety
/// The caller is required to supply the correct virtual address.
unsafe fn load_elf_segment(
    ...
    // SAFETY: the caller is required to supply the correct address for this
    // segment.
    let segment_buf =
...
fn load_kernel_elf(
    ...
    // SAFETY: the caller is required to provide the correct virtual address
    // range.
...
fn load_igvm_params(
    ...
    // SAFETY: the caller is required to specify the correct virtual addresses.
    let (igvm_src, igvm_dst) = unsafe {
        (
            slice::from_raw_parts(src_addr.as_ptr::<u8>(), igvm_vregion.len()),
