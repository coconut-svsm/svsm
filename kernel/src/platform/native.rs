...
impl SvsmPlatform for NativePlatform {
    ...
    fn start_cpu(
        ...
        // SAFETY: the address of the transition page is known to be valid
        // during the AP startup phase.
        unsafe {
            let size = mem::size_of::<ApStartContext>();
            let context_va = SIPI_STUB_GPA as usize + PAGE_SIZE - size;
            let context_ptr = VirtAddr::new(context_va).as_mut_ptr::<ApStartContext>();
            ptr::copy_nonoverlapping(&ap_context, context_ptr, size);
        }
