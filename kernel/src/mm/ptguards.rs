...
impl PerCPUPageMappingGuard {
    ...
    pub fn create(
        ...
        let mapping = if huge {
            ...
            with_pgtable_mut(|pg| pg.map_region_2m(range.region(), paddr_start, flags, false))?;
            range
        } else {
            let range = VRangeAlloc::new_4k(size, 0)?;
            with_pgtable_mut(|pg| pg.map_region_4k(range.region(), paddr_start, flags, false))?;
            range
        };
...
impl Drop for PerCPUPageMappingGuard {
    fn drop(&mut self) {
        ...
        let size = if self.mapping.huge() {
            with_pgtable_mut(|pg| pg.unmap_region_2m(region));
            PageSize::Huge
        } else {
            with_pgtable_mut(|pg| pg.unmap_region_4k(region));
            PageSize::Regular
        };
