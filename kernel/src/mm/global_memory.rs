...
use crate::cpu::percpu::with_pgtable_mut;
...
impl GlobalRangeGuard {
    ...
    fn map(&self, paddr: PhysAddr, flags: PTEntryFlags) -> Result<(), SvsmError> {
        if self.huge {
            with_pgtable_mut(|pg| pg.map_region_2m(self.region(), paddr, flags, self.shared))
        } else {
            with_pgtable_mut(|pg| pg.map_region_4k(self.region(), paddr, flags, self.shared))
        }
    }

    fn unmap(&self) {
        if self.huge {
            with_pgtable_mut(|pg| pg.unmap_region_2m(self.region()));
        } else {
            with_pgtable_mut(|pg| pg.unmap_region_4k(self.region()));
        }
    }
