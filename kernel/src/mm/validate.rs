...
impl ValidBitmap {
    ...
    fn migrate(&mut self, mut new: PageBox<[MaybeUninit<u64>]>) {
        for (dst, src) in new.iter_mut().zip(self.bitmap.iter()) {
            dst.write(*src);
        }
        for dst in new.iter_mut().skip(self.bitmap.len()) {
            dst.write(0);
        }
    ...
    fn set_2m(&mut self, paddr: PhysAddr, val: u64) {
        ...
        for dst in self.bitmap[index..index + NR_INDEX].iter_mut() {
            *dst = val;
        }
    }
    ...
    fn set_range(&mut self, paddr_begin: PhysAddr, paddr_end: PhysAddr, new_val: bool) {
        ...
        if index_head != index_tail {
            ...
            for val in self.bitmap[index_head + 1..index_tail].iter_mut() {
                *val = new_val;
            }
