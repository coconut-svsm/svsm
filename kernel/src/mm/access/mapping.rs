...
impl<A, T> OwnedMapping<A, T> {
    ...
    #[inline]
    pub(super) const fn as_borrow(&self) -> BorrowedMapping<'_, A, T> {
        BorrowedMapping {
    ...
    pub const fn data_vregion(&self) -> MemoryRegion<VirtAddr> {
...
impl<A, T> OwnedMapping<A, [T]> {
    ...
    /// trait methods reusing [`BorrowedMapping`] functionality.
    #[inline]
    pub(super) const fn as_borrowed_slice(&self) -> BorrowedMapping<'_, A, [T]> {
        let start = self.mem.region().start().const_add(self.off);
