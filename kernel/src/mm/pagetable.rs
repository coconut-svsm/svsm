...
impl PTEntry {
    ...
    /// Check if the page table entry is shared.
...
impl PageTable {
            ...
            .and_then(|(pte, leaf)| {
                if leaf {
                    ...
                    PageTable::translate_addr_lvl0(
                        PTPage::from_entry(pte).ok_or(attr.default_pf_err(mem_am))?,
                        vaddr,
                        mem_am,
                        attr,
                        pteflags,
    ...
    fn translate_addr(
        ...
        PageTable::translate_addr_lvl3(&self.root, vaddr, mem_am, attr, &mut pteflags).and_then(
            |(e, s)| {
                if let Some(size_mask) = s.checked_sub(1) {
                    Ok((e.address() + (vaddr.bits() & size_mask), e.shared(), s))
                } else {
                    Err(attr.default_pf_err(mem_am))
                }
            },
        )
    }
    ...
    pub fn translate_addr_region(
        ...
        let region = MemoryRegion::from_addresses(
            ...
            vaddr
                ...
                .ok_or((vaddr, attr.default_pf_err(mem_am)))?
                .page_align_up(),
        );
        ...
        for addr in region.iter_pages(PageSize::Regular) {
            if trans.0 == trans.1 {
                trans = self.translate_addr(addr, mem_am, attr).map_or_else(
                    |e| Err((addr, e)),
                    |(start, shared, ps)| {
