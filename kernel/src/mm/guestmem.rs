...
///   memory to read from.
/// * `dst`: The pointer to the beginning of the guest memory range to populate.
...
pub fn copy_from_guest(src: PhysAddr, dst: *mut u8, size: usize) -> Result<(), SvsmError> {
    ...
    // SAFETY: Only reads data from a region outside the SVSM. The Uninit->Init transmute
    // is safe since the entirety of the slice is populated.
    unsafe {
...
/// If the physical address region cannot be mapped, it returns `Err(SvsmError::Mem)`.
/// If the physical address region cannot be read, it returns `Err(SvsmError::Fault)`.
pub fn copy_to_guest(src: &[u8], dst: PhysAddr) -> Result<(), SvsmError> {
    let size = src.len();
...
/// If the physical address region cannot be mapped, it returns `Err(SvsmError::Mem)`.
/// If the physical address region cannot be read, it returns `Err(SvsmError::Fault)`.
pub fn read_from_guest<T: FromBytes + Sized>(src: PhysAddr) -> Result<T, SvsmError> {
    // Should be stack allocatable, but using size_of in array bounds is not possible
    // in current Rust. The zero, copy, read sequence could be better optimized too.
    let mut result_bytes = vec![0u8; size_of::<T>()];
    copy_from_guest(src, result_bytes.as_mut_ptr(), size_of::<T>())?;
    T::read_from_bytes(&result_bytes).map_err(|_| SvsmError::InvalidBytes)
}
