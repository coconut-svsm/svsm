...
pub unsafe fn read_u8(v: VirtAddr) -> Result<u8, SvsmError> {
    ...
    // functions safety requirements are fulfilled.
    unsafe {
        asm!("1: movb ({0}), %al",
             "   xorq %rcx, %rcx",
             "2:",
...
pub unsafe fn write_u8(v: VirtAddr, val: u8) -> Result<(), SvsmError> {
    ...
    // SAFETY: Assembly writes to virtual address, safe when functions safety
...
unsafe fn copy_bytes(src: *const u8, dst: *mut u8, size: usize) -> Result<(), SvsmError> {
    ...
    // SAFETY: Safe as long as the functions safety requirements are met. Any
