...
///   memory to read from.
/// * `dst`: The byte slice to populate from guest memory.
///
/// # Returns
///
...
pub fn copy_from_guest(src: PhysAddr, dst: &mut [u8]) -> Result<(), SvsmError> {
    let size = dst.len();
    let region = checked_guest_region(src, dst.len())?;
...
pub fn read_bytes_from_guest(src: PhysAddr, size: usize) -> Result<Vec<u8>, SvsmError> {
    ...
    unsafe {
        copy_from_guest(src, result.as_mut_slice())?;
        result.set_len(size);
    }
    Ok(result)
}
...
pub fn read_from_guest<T: FromBytes + Sized>(src: PhysAddr) -> Result<T, SvsmError> {
    ...
    copy_from_guest(src, &mut result_bytes[..])?;
