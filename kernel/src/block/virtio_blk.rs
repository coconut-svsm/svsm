...
mod tests {
    ...
    fn get_image_hash_from_host() -> Option<[u8; 32]> {
        ...
        sp.put_byte(IORequest::GetStateImageMd5 as u8);
    ...
    fn virtio_read(sectors_at_once: usize) {
        ...
        while sector < n_sectors {
            if n_sectors - sector < buffer.len() {
                buffer.truncate(SECTOR_SIZE);
            }

            blk.read_blocks(sector, &mut buffer).unwrap();
            hasher.update(&buffer);
            sector += buffer.len() / SECTOR_SIZE;
        }
    ...
    fn virtio_write(sectors_at_once: usize) {
        ...
        while sector < n_sectors {
            ...
            blk.write_blocks(sector, &buffer).unwrap();
            hasher.update(&buffer);
            sector += buffer.len() / SECTOR_SIZE;
        }
