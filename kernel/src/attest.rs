...
use crate::{
    ...
    serial::SerialPort,
    utils::vec::{try_to_vec, u8_vec_sized},
};
...
impl AttestationDriver<'_> {
    ...
    fn read(&mut self) -> Result<Vec<u8>, AttestationError> {
        ...
        let mut buf = u8_vec_sized(len).or(Err(AttestationError::VecAlloc))?;
...
fn evidence(tee: &Tee, hash: Vec<u8>) -> Result<Vec<u8>, AttestationError> {
    let evidence = match tee {
        &Tee::Snp => {
            ...
            let mut buf = u8_vec_sized(2048).or(Err(AttestationError::VecAlloc))?;
