...
pub trait VsockDriver: Sync + Send {
    ...
    /// # Parameters
    ///
    /// * `remote_cid` - The Context ID of the remote endpoint
    ...
    fn shutdown(&self, remote_cid: u64, local_port: u32, remote_port: u32)
        -> Result<(), SvsmError>;

    /// Forcibly terminates a vsock connection.
    ///
    /// Immediately terminates the connection without waiting for peer confirm.
    ///
    /// # Parameters
    ///
    /// * `remote_cid` - The CID of the remote endpoint
    ...
    /// # Returns
    ///
    /// * `Ok(())` if the forced shutdown was successful
    /// * `Err(SvsmError)` if the shutdown failed
    fn force_shutdown(
        &self,
        remote_cid: u64,
    ...
    /// Returns the first available local port number.
    ///
    /// # Returns
    ///
    /// * `Ok(u32)` - An available port number
    /// * `Err(SvsmError)` if no free port could be found
