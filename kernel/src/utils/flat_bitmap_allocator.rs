...
impl<const N: usize, const WORDS: usize> FlatBitmapAllocator<N, WORDS> {
    ...
    /// Returns `true` if `bit` is currently marked used.
    pub fn is_used(&self, bit: usize) -> bool {
        assert!(bit < N);
        let word = self.words[bit / (u64::BITS as usize)].load(Ordering::Acquire);
        (word & (1u64 << (bit % (u64::BITS as usize)))) != 0
    }
}
...
mod tests {
    ...
    #[test]
    fn alloc_and_free() {
        let b = <flat_bitmap_allocator!(128)>::new();
    ...
    #[test]
    fn exhausted() {
        let b = <flat_bitmap_allocator!(64)>::new();
        for _ in 0..64 {
            assert!(b.alloc().is_some());
        }
    ...
    fn partial_capacity_padding() {
        let b = <flat_bitmap_allocator!(100)>::new();
        let mut n = 0;
