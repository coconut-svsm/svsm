...
impl<const T: usize> StringRingBuffer<T> {
    ...
    pub fn write(&mut self, s: impl Iterator<Item = char>) {
        s.for_each(|c| self.write_char(c));
    }
...
impl<const T: usize> Default for StringRingBuffer<T> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_ring_wrap() {
        let mut ring = StringRingBuffer::<32>::new();
        ring.write("0000000000000000".chars());
        ring.write("1111111111111111".chars());
        ring.write("2222222222222222".chars());
    ...
    fn test_ring_overflow() {
        let mut ring = StringRingBuffer::<32>::new();
        ring.write("000000000000000011111111111111112222222222222222".chars());
    ...
    fn test_ring_second_wrwr() {
        let mut ring = StringRingBuffer::<32>::new();
        ring.write("Testing1".chars());
        let s = ring.read();
        assert!(s.is_some());
        assert_eq!(s.unwrap(), "Testing1");

        ring.write("Testing2".chars());
        let s = ring.read();
        assert!(s.is_some());
