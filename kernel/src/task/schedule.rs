    ...
    unsafe {
        let cr3 = {
            ...
            if let Some(pcid) = (*next).pcid() {
                ...
                // spaces with a PCID across context switches.
                //
                // This is safe only because task VMR unmaps shoot down the
                // owning PCID on every CPU (INVPCID type 0/1 or INVLPGB).
                // INVLPG is not sufficient: it invalidates only the current
                // PCID, so a migrated task's stale tags would otherwise
                // survive on the previous CPU.
                cr3 |= 1usize << 63;
            }
            cr3
        };
