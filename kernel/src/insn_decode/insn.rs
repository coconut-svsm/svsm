...
pub mod test_utils {
    ...
    impl InsnMachineMem for TestMem {
        fn read_integer(&self, offset: usize, size: Bytes) -> Result<u64, InsnError> {
            let mut buf = [0u8; 8];

            if (size as usize > buf.len()) || (self.len < offset + size as usize) {
                return Err(InsnError::MemRead);
            }

            // SAFETY: The buf length has been checked. The offset and size have been
            // checked as well.
            unsafe {
                self.ptr
                    .add(offset)
                    .copy_to_nonoverlapping(buf.as_mut_ptr(), size as usize);
            }
        ...
        fn write_integer(
            ...
            unsafe {
                self.ptr
                    .add(offset)
                    .copy_from_nonoverlapping(buf.as_ptr(), size as usize);
            }
