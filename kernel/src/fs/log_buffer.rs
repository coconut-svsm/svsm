...
impl LogFile {
    fn new<S: AsRef<str>>(component: S) -> Self {
        Self {
            lb: SpinLock::new(LineBuffer::new(component)),
        }
    }
}
...
pub fn stdout_open(taskname: &str) -> Arc<dyn Obj> {
    let log_file: Arc<dyn File> = Arc::new(LogFile::new(taskname));
