...
impl PerCpu {
    ...
    pub fn schedule_init(&self) -> TaskPointer {
        // Ensure that interrupts will be enabled on the current CPU when
        // leaving the scheduler environment.  This is done after disabling
        // interrupts for scheduler initialization so that the first interrupt
        // that can be received will always observe that there is a current
        // task and not the boot thread.
        unsafe {
