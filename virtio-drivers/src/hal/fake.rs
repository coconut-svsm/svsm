...
unsafe impl Hal for FakeHal {
    // SAFETY: Memory allocated with this function is valid, and properly aligned.
    ...
    /// SAFETY: Caller must ensure vaddr was returned by a previous call to dma_alloc and the
    ...
    /// SAFETY: Caller must ensure paddr is a valid MMIO region of the given size.
    unsafe fn mmio_phys_to_virt(paddr: PhysAddr, _size: usize) -> NonNull<u8> {
        NonNull::new(paddr as _).unwrap()
    }

    /// SAFETY: The buffer must be a valid pointer to a non-empty memory range which will not be accessed by
