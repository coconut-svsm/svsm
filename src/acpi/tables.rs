...
impl RSDPDesc {
    ...
    pub fn load(&mut self, fw_cfg : &FwCfg) -> Result<(), ()> {
        ...
        if file.size() as usize != mem::size_of::<RSDPDesc>() { return Err(()); }
...
fn array_to_string<const S : usize>(data : [u8; S]) -> FixedString<S> {
    ...
    for i in 0..S { fs.push(data[i] as char); }
...
impl ACPITableHeader {
    ...
    pub fn print_summary(&self) {
        ...
        println!("ACPI: [{} {} {} {} {} {} {} {} {}]", sig, self.len, self.rev,
                 self.chksum, oem_id, oem_table_id, self.oem_rev,
                 compiler_id, self.compiler_rev);
    }
}
...
impl ACPITableBuffer {
    ...
    pub fn load_from_fwcfg(&mut self, fw_cfg : &FwCfg) -> Result<(), ()> {
        ...
        self.load_tables(fw_cfg).expect("Loading ACPI tables failed");
...
pub fn load_acpi_cpu_info(fw_cfg : &FwCfg) -> Result<Vec<ACPICPUInfo>, ()> {
    ...
    let mut apic_table = buffer.acp_table_by_sig("APIC").expect("MADT ACPI table not found");
