...
impl SevFWMetaData {
    ...
    pub fn add_valid_mem(&mut self, base : PhysAddr, len : usize) {
        self.valid_mem.push(SevPreValidMem { base : base, length : len });
    }
}
...
unsafe fn find_table(uuid : &Uuid, start : VirtAddr, len : VirtAddr) -> Result<(VirtAddr, usize), ()> {
    ...
    while curr >= end {
        ...
        if len < mem::size_of::<Uuid>() + mem::size_of::<u16>() { break; }
...
pub fn parse_fw_meta_data() -> Result<SevFWMetaData, ()> {
    ...
    unsafe {
        ...
        if let Ok(tbl) = ret {
            let (base, len) = tbl;
            if len != mem::size_of::<u32>() { return Err(()); }
