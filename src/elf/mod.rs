...
///
/// let range1 = Elf64AddrRange { vaddr_begin: Elf64Addr(0x1000), vaddr_end: Elf64Addr(0x1100) };
/// let range2 = Elf64AddrRange { vaddr_begin: Elf64Addr(0x1100), vaddr_end: Elf64Addr(0x1200) };
///
...
impl<'a> Elf64File<'a> {
    ...
    /// let file_range = elf_file.map_vaddr_to_file_off(vaddr_begin, Some(vaddr_end));
    /// match file_range {
    ///     Ok(range) => {
    ///         println!("File Offset Range: {} to {}", range.offset_begin, range.offset_end);
    ///     },
    ///     Err(error) => {
    ///         println!("Error: {:?}", error);
    ///     }
    ...
    /// match elf_file.map_vaddr_to_file_buf(vaddr_begin, Some(vaddr_begin + 0x100)) {
    ///     Ok(slice) => {
    ...
    ///     println!("Segment at 0x{:x} - 0x{:x}", segment.vaddr, segment.vaddr + segment.memsz);
    /// }
    /// ```
    ...
    ///
    /// match elf_file.apply_dyn_relas(relocation_processor, image_load_addr) {
...
impl Elf64Hdr {
    ...
    /// let file_contents: Vec<u8> = std::fs::read("my_elf_file.bin").unwrap();
