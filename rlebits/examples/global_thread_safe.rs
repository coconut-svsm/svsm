...
fn main() {
    ...
    GLOBAL_BITS.dump_with(|start| format!("start={:#x}", start << 12));
    ...
    println!(
        "Last bit (2^52-1): {:?}",
        GLOBAL_BITS.get((1_usize << 52) - 1)
    );
    println!("Sanity check: {}", GLOBAL_BITS.sanity_check());
    #[cfg(any(feature = "std", test))]
    GLOBAL_BITS.dump_with(|start| format!("start={}", start));
    ...
    println!(
        "Sanity check after range set: {}",
        GLOBAL_BITS.sanity_check()
    );
