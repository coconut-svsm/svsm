...
// Create a bit array that can hold values 0-99
let mut bits = RleBits::new(100);
...
let bits = ThreadSafeRleBits::new(100);
...
let bits = Arc::new(ThreadSafeRleBits::new(1000));
let mut handles = vec![];
...
let mutex_bits = ThreadSafeRleBits::new(1000);

// 2. RwLockRleBits - Optimized for read-heavy workloads
let rw_bits = RwLockRleBits::new(1000);

// 3. ShardedRleBits - Multiple independent shards for spatial locality
let sharded_bits = ShardedRleBits::new(1000, 4); // 4 shards

// 4. EventuallyConsistentRleBits - Thread-local with periodic sync
let eventually_bits = EventuallyConsistentRleBits::new(1000);
```

**Performance characteristics** (from benchmarks):
- `ShardedRleBits`: 6.3x faster than mutex (best for spatially separated access)
...
- `EventuallyConsistentRleBits`: Fastest for thread-local access (requires std)
